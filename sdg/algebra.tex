...
\paragraph{More General Free Algebras}
The forgetful functor from algebras to the base universe has a left adjoint, giving the free algebra construction on any type.
This can be explicitly constructed as a quotient of a set of terms.  \rednote{Expand this}

Given that we now have free algebras and quotients, we can see that every \(\EE_X\)-algebra \(A\) can be specified by some generators and relations,
simply by taking $F(A)/R$ where $sRt$ iff $s = t$ when interpereted into $A$ in the canonical way.

\paragraph{Coproducts}
In light of the previous remark, given two algebras $F(X)/R$ and $F(Y)/S$ we can calculate that their coproduct is
$F(X \sqcup Y) / (R \sqcup S)$.
...
\begin{example}
  ...
  \[ f : \Z \to \Z \]
  \[ f(0) = 0 ; f(n) = 1 \]
  Then $0 \sim 2$ but $f(0)$ and $f(1)$ are not related.
\end{example}
...
\begin{example}
  In the Dubuc topos \(\mathrm{Dub}\), the smooth reals \(R = \yo \ell C^\infty(\R)\) form a Fermat-Hadamard ring.
...
\begin{proposition}
  ...
  \rednote{To make this true, we just assume $X$ is regular. There might be a locality condition that lets us deduce this?}
  Let \(R\) be a ring.
...
\begin{definition}
  Given an \(\EE_R\)-Algebra \(A\) and \(f:A\), we can define the localisation of \(A\) at \(f\) as
  \[ A_f := (A \otimes F(1)) / (fX - 1)\]
  For clarity we write \(A \otimes F(n)\) as \(A\{X_1, \ldots, X_n\}\).
\end{definition}
}
